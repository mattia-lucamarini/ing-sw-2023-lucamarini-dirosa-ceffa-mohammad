...
public class GUIClient {
    ...
    public GUIClient(View view){
        this.view = view;
    }

    public void data(){
        viewhandler = view.getViewhandler();
        userInterface = new GUIInterface(stage,viewhandler);
    ...
    private boolean login(Socket socket) {
        ...
        if (message.getMessageType().equals(MessageCode.NUM_PLAYERS_REQUEST)) {
            try{view.showNumScreen(stage);
                userInterface.askForNumOfPlayers(clientHandler);}
            catch(IOException e){
                userInterface.printErrorMessage(("Error in asking num of players"));
            }
            try{
                userInterface.askForNumOfPlayers(clientHandler);
            } catch(IOException e){
